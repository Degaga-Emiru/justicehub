...
import { UploadCloud, FileText, Trash2, Plus, Loader2, Download, AlertTriangle, Shield, ExternalLink } from "lucide-react";
import { useQuery, useMutation, useQueryClient } from "@tanstack/react-query";
import { fetchCases, fetchCitizenCaseDocuments, uploadCitizenDocument, downloadCitizenDocumentVersion, viewCitizenDocumentVersion, deleteCitizenDocument, fetchDefendantCases } from "@/lib/api";
...
export default function DocumentsPage() {
  ...
  const handleDownload = async (versionId, fileName) => {
    try {
      await downloadCitizenDocumentVersion(versionId, fileName);
                    ...
                    {activeVersion && (
                      <>
                        <Button
                          size="sm"
                          variant="ghost"
                          className="flex-1 bg-muted/50 text-foreground hover:bg-muted font-bold text-xs h-9"
                          onClick={() => handleView(activeVersion.id)}
                        >
