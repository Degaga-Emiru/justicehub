...
import { 
  ...
  Loader2, Info, FileText, CreditCard, Trash2, ShieldAlert
} from "lucide-react";
import {
  Dialog,
  DialogContent,
...
export default function AdminNotificationsPage() {
  ...
  const getIcon = (type) => {
    switch (type) {
      case 'SYSTEM_ALERT': return <ShieldAlert className="h-5 w-5 text-rose-500" />;
    ...
    <div className="space-y-8 animate-fade-up max-w-4xl mx-auto pb-20 p-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground font-medium text-lg">System-wide alerts, security logs, and administrative updates.</p>
        </div>
      ...
      <div className="space-y-4">
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-20 space-y-4">
            ...
            <p className="text-xs font-black uppercase tracking-widest text-muted-foreground">Synchronizing feed...</p>
          </div>
          ...
          <>
            <div className="flex items-center justify-between px-2 py-1">
              ...
              >
                {selectedIds.length === notifications?.length ? "Deselect All" : "Select All Notifications"}
              </Button>
            </div>
            {notifications.map((notif) => (
              <div key={notif.id} className="flex gap-4 items-center animate-in slide-in-from-left duration-300">
                <Checkbox 
                ...
                <Card 
                  className={cn(
                    ...
                    !notif.is_read ? "border-l-4 border-l-primary bg-primary/5 shadow-md" : "opacity-75"
                  )}
                >
          ...
          <div className="flex flex-col items-center justify-center py-40 space-y-6">
            <div className="h-24 w-24 rounded-[2.5rem] bg-muted/20 flex items-center justify-center rotate-3 border border-border shadow-inner animate-pulse">
              <Bell className="h-10 w-10 text-muted-foreground/20" />
            </div>
            <div className="text-center space-y-2">
              <p className="text-2xl font-black font-display text-foreground uppercase tracking-tight">Ecosystem Quiet</p>
              <p className="text-sm text-center max-w-[320px] text-muted-foreground font-medium">No administrative alerts currently require your intervention.</p>
            </div>
          </div>
        )}
      </div>
      ...
      <Dialog open={isBulkDeleting} onOpenChange={setIsBulkDeleting}>
        <DialogContent className="sm:max-w-md rounded-[2rem] border-border shadow-2xl">
          <DialogHeader>
            ...
            <DialogDescription className="text-base font-medium pt-3">
              You are about to delete {selectedIds.length} notifications.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="flex gap-3 sm:justify-end pt-8">
