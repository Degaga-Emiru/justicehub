...
export default function AdminCategoriesPage() {
 ...
 <Input id="name" name="name" required placeholder="e.g., Civil Law" className="h-12 bg-background border-border rounded-xl" />
 </div>
 <div className="space-y-2">
 <Label htmlFor="code" className="text-xs font-black uppercase tracking-widest text-muted-foreground">Category Code</Label>
 <Input id="code" name="code" required placeholder="e.g., CVL" className="h-12 bg-background border-border rounded-xl" />
 </div>
 <div className="space-y-2">
 <Label htmlFor="fee" className="text-xs font-black uppercase tracking-widest text-muted-foreground">Filing Fee (ETB)</Label>
 <Input id="fee" name="fee" type="number" step="0.01" defaultValue="0" className="h-12 bg-background border-border rounded-xl" />
 </div>
 <div className="space-y-2">
 <Label htmlFor="description" className="text-xs font-black uppercase tracking-widest text-muted-foreground">Description</Label>
 <Textarea id="description" name="description" placeholder="Brief description..." className="bg-background border-border rounded-xl" />
 </div>
 ...
 <TableRow key={category.id} className="border-border hover:bg-muted/30 transition-colors group">
 ...
 <Button variant="outline" size="sm" className="rounded-xl font-bold text-xs border-border hover:bg-primary/10 hover:text-primary hover:border-primary/20 transition-all gap-2" onClick={() => setSelectedCategory(category)}>
...
function CategoryJudgesPanel({ category, onClose }) {
 ...
 <p className="text-muted-foreground font-medium text-lg leading-relaxed flex items-center gap-2 mt-2">
 ...
 <div className="rounded-2xl border border-border bg-card/30 bg-background shadow-sm border-border overflow-hidden shadow-2xl p-8">
 <div className="flex justify-between items-center mb-6">
 <h3 className="font-black font-display tracking-tight text-xl flex items-center gap-2">
 <Gavel className="h-6 w-6 text-primary" />
 Assigned Judges
 </h3>
 <Badge variant="outline" className="font-mono text-sm bg-muted/30 border-border px-3 py-1">
 {judges?.length || 0} Total
 </Badge>
 </div>

 {isJudgesLoading ? (
 <div className="flex flex-col items-center justify-center p-12 border border-dashed border-border rounded-2xl bg-muted/10">
 ...
 <div className="h-16 w-16 rounded-[1.5rem] bg-muted/20 flex items-center justify-center mb-4 border border-border">
 ...
 return (
 <div key={judge.id} className="p-6 border border-border rounded-3xl bg-muted/30 hover:border-primary/20 hover:bg-muted/50 transition-all flex flex-col gap-6 group shadow-lg">
 ...
 <p className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">Active Workload</p>
 <div className="flex items-baseline gap-1">
 <span className="text-xl font-black font-mono text-foreground">{currentCases}</span>
 <span className="text-sm text-slate-500 font-bold">/</span>
 <span className="text-sm text-muted-foreground font-mono font-bold">{maxCases} Cases</span>
 </div>
 </div>
