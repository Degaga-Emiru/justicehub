...
import { Bell, CheckCircle, Trash2, Clock, Calendar, AlertTriangle, Loader2, Info } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogDescription,
  ...
  return (
    <div className="space-y-8 animate-fade-up max-w-4xl mx-auto pb-20 p-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold font-display tracking-tight text-[#1A202C] mb-2">Notifications</h1>
          <p className="text-[#4A5568] font-bold opacity-100">Clear communication on your case progression.</p>
        </div>
      ...
      {notifications?.length > 0 && (
        <div className="flex items-center gap-2 px-2 py-1">
          <Button 
            variant="ghost" 
            size="sm" 
            className="text-[10px] font-bold uppercase tracking-widest text-[#2D3748] hover:text-primary p-0 opacity-100"
      ...
      <div className="space-y-4">
        {isLoading ? (
            ...
            <div key={notif.id} className="flex gap-4 items-center">
              ...
              <Card 
                className={cn(
                  ...
                  !notif.is_read ? "border-l-4 border-l-primary bg-primary/5" : "opacity-100"
                )}
              >
                <CardContent className="p-6 relative">
                  <div className="flex gap-6">
                    ...
                    <div className="flex-1 space-y-1 pr-10">
                      <div className="flex justify-between items-start">
                        <h3 className={cn("text-base font-bold", !notif.is_read ? "text-[#1A202C]" : "text-[#4A5568]")}>
                          {notif.title}
                        </h3>
                        <span className="text-[10px] font-bold text-[#4A5568] uppercase py-1 opacity-100">
                          {notif.created_at ? format(new Date(notif.created_at), "MMM d, HH:mm") : "Just now"}
                        </span>
                      </div>
                      <p className="text-sm font-bold text-[#4A5568] leading-relaxed opacity-100">
                  ...
                  <Button 
                    variant="ghost" 
                    size="icon" 
                    className="absolute top-6 right-6 h-9 w-9 rounded-xl opacity-0 group-hover:opacity-100 hover:bg-rose-500/10 hover:text-rose-600 transition-all text-[#4A5568]"
          ...
          <div className="flex flex-col items-center justify-center py-40 space-y-4 ">
            ...
            <p className="text-lg font-bold text-[#1A202C] opacity-100">Your feed is clear</p>
            <p className="text-sm text-center max-w-[300px] text-[#4A5568] font-bold opacity-100">We'll notify you here when the court takes any action on your cases.</p>
          </div>
        )}
      </div>
      ...
      <Dialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
        <DialogContent className="sm:max-w-md rounded-3xl border-border shadow-2xl">
          <DialogHeader>
            ...
            <DialogDescription className="text-sm font-bold pt-2 text-[#4A5568] opacity-100">
